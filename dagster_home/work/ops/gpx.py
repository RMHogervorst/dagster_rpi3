...
def read_gpx_file(file: str) -> (pd.DataFrame, Dict[str, str]):
...
def parse_tracks(tracksdict):
    ...
    for segment in tracksdict[0].get(a + "trkseg"):
        for point in segment.get(a + "trkpt"):
            ...
            speed_.append(
                point.get(a + "extensions", [{"{https://osmand.net}speed": nullvalue}])[
                    0
                ].get(o + "speed", nullvalue)[0]["_text"]
            )
            heading_.append(
                point.get(
                    a + "extensions", [{"{https://osmand.net}heading": nullvalue}]
                )[0].get(o + "heading", nullvalue)[0]["_text"]
            )
    # stop
    result = pd.DataFrame(
        {
